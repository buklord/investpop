...
export const BASE_PRICES = {
  ...
  // Commodities
  XAUUSD: 2320.00, XAGUSD: 27.50, USOIL: 83.00, XPTUSD: 950.00, NATGAS: 1.85,
}

// Default pip sizes per symbol (used to compute spread in price units)
// NOTE: We must not treat all 6-letter `*USD` symbols as FX.
// Examples like ADAUSD/BTCUSD are crypto quoted in USD and need a much smaller
// spread unit, otherwise bid can go negative and charts look empty/invalid.
const FIAT_CODES = new Set(['USD', 'EUR', 'GBP', 'JPY', 'CHF', 'CAD', 'AUD', 'NZD'])
function getPipSize(symbol) {
  const sym = String(symbol || '').toUpperCase()
  if (!sym) return 0.01

  // FX pairs: both base and quote are fiat codes.
  if (sym.length === 6) {
    ...
    // Common non-FX 6-letter symbols quoted in USD (crypto / synthetic pairs)
    if (quote === 'USD') return 0.01
  }

  // Indices usually contain digits (US30, SPX500, GER40, etc.)
  if (/[0-9]/.test(sym)) return 1

  // Stocks/crypto default
  return 0.01
}
...
const SIM = {
  ...
  history: {},         // { [symbol]: Array<{ t, mid, bid, ask }> }
  initialized: false,
  settings: {
...
const SEED_TICKS    = 500   // pre-generated history ticks on cold start
const TICK_INTERVAL = 2000  // ms between ticks (matches client poll interval)

function pushHistory(symbol, point) {
  const sym = symbol.toUpperCase()
  if (!SIM.history[sym]) SIM.history[sym] = []
  SIM.history[sym].push(point)
  if (SIM.history[sym].length > HISTORY_LIMIT) {
    SIM.history[sym] = SIM.history[sym].slice(-HISTORY_LIMIT)
  }
}
...
export function tick() {
  ...
  for (const [symbol, base] of Object.entries(BASE_PRICES)) {
    ...
    if (override && override.expiresAt > Date.now()) {
      ...
      const prev = SIM.prices[symbol]
      const high = Math.max(prev?.high ?? override.price, override.price)
      const low = Math.min(prev?.low ?? override.price, override.price)
      updates[symbol] = { mid: override.price, bid, ask, high, low, source: 'OVERRIDE', updatedAt: Date.now() }
      SIM.prices[symbol] = updates[symbol]
      pushHistory(symbol, { t: Date.now(), mid: override.price, bid, ask })
      continue
    }
    // Random walk
    ...
    const prev = SIM.prices[symbol]
    const high = Math.max(prev?.high ?? mid, mid)
    const low = Math.min(prev?.low ?? mid, mid)
    updates[symbol] = { mid, bid, ask, high, low, source: 'SIMULATED', updatedAt: Date.now() }
    SIM.prices[symbol] = updates[symbol]
    pushHistory(symbol, { t: Date.now(), mid, bid, ask })
  }
  return updates
}
...
/**
 * Generate SEED_TICKS historical ticks for a single symbol using a random walk
 * that starts from `base` and winds forward to roughly the same price.
 * Timestamps are spaced TICK_INTERVAL ms apart ending just before `now`.
 */
function seedHistory(symbol, base) {
  const { volatility, trendBias, spreadPips } = SIM.settings
  const drift     = trendBias === 'BULL' ? 0.0003 : trendBias === 'BEAR' ? -0.0003 : 0
  const noiseScale = 0.001 * volatility
  const now        = Date.now()
  const startTime  = now - SEED_TICKS * TICK_INTERVAL  // furthest point in the past

  let mid = base
  for (let i = 0; i < SEED_TICKS; i++) {
    const noise      = (Math.random() * 2 - 1) * noiseScale * mid
    const rawChange  = drift * mid + noise
    mid = parseFloat((mid + clampChange(mid, rawChange)).toFixed(6))
    if (mid <= 0) mid = base
    const { bid, ask } = deriveBidAsk(mid, symbol, spreadPips)
    pushHistory(symbol, { t: startTime + i * TICK_INTERVAL, mid, bid, ask })
  }
  return mid   // return the final mid price
}

/** Return all current prices (initialise if needed) */
export function getAllPrices() {
  if (!SIM.initialized) {
    // Seed from base prices on first call – also pre-fill chart history
    for (const [symbol, base] of Object.entries(BASE_PRICES)) {
      if (!SIM.prices[symbol]) {
        const seededMid = seedHistory(symbol, base)
        const { bid, ask } = deriveBidAsk(seededMid, symbol, SIM.settings.spreadPips)
        SIM.prices[symbol] = {
          mid: seededMid,
          bid,
          ask,
...
export function getHistory(symbol, limit = HISTORY_LIMIT) {
  getAllPrices()
  const sym = symbol?.toUpperCase()
  const arr = SIM.history[sym] || []
