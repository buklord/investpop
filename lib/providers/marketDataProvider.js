...
// Market trend bias — updated from DB by the admin. Module-level so it persists
// across requests within the same Node.js process.
let MARKET_TREND = 'NEUTRAL'  // 'BULL' | 'BEAR' | 'NEUTRAL'
let trendLastFetchMs = 0

/** Called by the route handler to sync the market trend from DB */
export function setMarketTrend(trend) {
  if (typeof trend === 'string' && ['BULL', 'BEAR', 'NEUTRAL'].includes(trend)) MARKET_TREND = trend
}

/**
 * Generate a simulated quote by applying a random walk biased by the
 * current market trend. BULL → slight upward drift, BEAR → downward.
 * Caches the result so the next call continues the walk.
 */
function generateSimulatedQuote(symbol, type) {
  ...
  // Trend bias: BULL +0.15% drift, BEAR -0.15% drift, NEUTRAL 0
  const drift = MARKET_TREND === 'BULL' ? 0.0015 : MARKET_TREND === 'BEAR' ? -0.0015 : 0
  const noise = Math.random() * 0.006 - 0.003  // ±0.3% random noise
  const price = parseFloat((basePrice * (1 + drift + noise)).toFixed(6))
  const result = {
    symbol, type, price,
    changePercent: parseFloat((drift * 100 + (Math.random() * 6 - 3)).toFixed(2)),
...
class MockProvider {
  ...
  _makeQuote(symbol, type) {
    ...
    const drift = MARKET_TREND === 'BULL' ? 0.002 : MARKET_TREND === 'BEAR' ? -0.002 : 0
    const noise = Math.random() * 0.02 - 0.01
    return {
      symbol, type,
      price: mockData.price * (1 + drift + noise),
      changePercent: parseFloat(((drift + noise) * 100).toFixed(2)),
      timestamp: new Date().toISOString(),
      name: mockData.name,
    }
  }
