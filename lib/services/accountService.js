...
export class AccountService {
  ...
  async getAccount() {
    const accounts = await prisma.$queryRaw`
      SELECT id, balance, currency, COALESCE(margin_reserved, 0) as margin_reserved, created_at, updated_at
  ...
  async getAccountSummary() {
    ...
    // Balance  = cash in account (already reflects deducted trade costs)
    // Equity   = Balance + Open P&L  (live total if all positions closed now)
    // Margin Reserved = tracked DB value of capital locked in open positions
    // Available = Balance - Margin Reserved
    const dbMarginReserved = parseFloat(account.margin_reserved) || 0
    const marginReserved = dbMarginReserved > 0 ? dbMarginReserved : positionsValue
