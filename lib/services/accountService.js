...
export class AccountService {
  ...
  async getAccount() {
    ...
    if (accounts.length === 0) {
      ...
      await prisma.$executeRaw`
        ...
        VALUES (${id}::uuid, ${this.userId}::uuid, ${TRADING_CONFIG.STARTING_BALANCE})
      `
      return {
        id,
  ...
  async getAccountSummary() {
    ...
    const closedPositions = await prisma.$queryRaw`
      ...
      WHERE user_id = ${this.userId}::uuid AND status = 'CLOSED'
    `
  ...
  async createSnapshot(tradeId, snapshotType = 'TRADE') {
    ...
    await prisma.$executeRaw`
      ...
      VALUES (
        ${id}::uuid, ${this.userId}::uuid, ${summary.equity}, ${summary.balance},
        ${summary.positionsValue}, ${summary.openPnl}, ${summary.realizedPnl},
        ${tradeId}::uuid, ${snapshotType}
      )
    `
  ...
  async updateDailyPerformance(tradeFeePaid = 0) {
    ...
    const existing = await prisma.$queryRaw`
      ...
      WHERE user_id = ${this.userId}::uuid AND date = ${today}::date
    `
    ...
    if (existing.length > 0) {
      ...
      await prisma.$executeRaw`
        ...
        VALUES (
          ${id}::uuid, ${this.userId}::uuid, ${today}::date, ${summary.equity},
