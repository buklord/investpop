...
// Standalone helper — returns the 4 dashboard card values using the correct formulas
// balance   = virtual_accounts.balance + open positions cost (cash + locked capital)
// available = virtual_accounts.balance (free cash, after trade costs deducted)
// openPnl   = sum of unrealised P&L on open positions
// equity    = balance + openPnl  (live total account value)
export async function getAccountSnapshot(userId) {
  try {
    ...
    const balance   = (summary.balance || 0) + (summary.positionsValue || 0)
    const available = summary.balance || 0
    const openPnl   = summary.openPnl || 0
    const equity    = balance + openPnl
    return { balance, available, openPnl, equity, positionsCount: summary.positionsCount || 0, realizedPnl: summary.realizedPnl || 0 }
  } catch {
    return { balance: 0, available: 0, openPnl: 0, equity: 0, positionsCount: 0, realizedPnl: 0 }
  }
}
