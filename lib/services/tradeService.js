...
export class TradeService {
  ...
  async executeBuy(params) {
    ...
    if (existingPosition) {
      ...
      // Only set leader_position_id if provided (defensive during migration)
      if (leaderPositionId) {
        await prisma.$executeRaw`
          ...
          SET quantity = ${newQuantity}, 
              ...
              leverage = ${leverage},
              leader_position_id = COALESCE(leader_position_id, ${leaderPositionId})
          WHERE id = ${positionId}
        `
      } else {
        await prisma.$executeRaw`
      ...
      // Create new position
      positionId = uuidv4()
      // Only include leader_position_id if provided (defensive during migration)
      if (leaderPositionId) {
        await prisma.$executeRaw`
          ...
          VALUES (
            ...
            ${leverage}, 'OPEN', ${totalValue}, ${fee}, ${accountType}, ${leaderPositionId}
          )
        `
      } else {
        await prisma.$executeRaw`
    ...
    console.log(`[DEBUG] executeBuy: About to trigger copy trades with positionId=${positionId}`)
  ...
  async executeSell(params) {
    ...
    console.log(`[DEBUG] executeSell: isCopyClose=${isCopyClose}, remainingQuantity=${remainingQuantity}, positionId=${existingPosition.id}`)
    if (!isCopyClose && remainingQuantity <= 0.0000001) {
      console.log(`[DEBUG] Calling triggerCopyCloses for position ${existingPosition.id}`)
      this.triggerCopyCloses({
        ...
        console.error('[CopyTrading] Background copy close trigger failed:', err?.message)
        console.error('[CopyTrading] Full error:', err)
      })
    } else {
      console.log(`[DEBUG] Skipping triggerCopyCloses: isCopyClose=${isCopyClose}, remainingQuantity=${remainingQuantity}`)
    }
  ...
  async triggerCopyTrades(tradeData) {
    try {
      console.log(`[DEBUG] triggerCopyTrades called with data:`, tradeData)
      // Trigger for any user who has active followers (not admin-only)
      const followerCheck = await prisma.$queryRaw`
        SELECT COUNT(*)::int AS cnt FROM copy_trading_followers
        WHERE leader_id = ${this.userId} AND status = 'ACTIVE'
      `
      const hasFollowers = parseInt(followerCheck?.[0]?.cnt || 0) > 0
      console.log(`[DEBUG] User ${this.userId} active followers: ${followerCheck?.[0]?.cnt}`)
      if (!hasFollowers) {
        console.log(`[DEBUG] No active followers, skipping copy trades`)
        return
      }
      ...
      console.log(`[DEBUG] Calling executeCopyTrades with leaderId=${this.userId}, leaderPositionId=${tradeData.leaderPositionId}`)
  ...
  // Trigger copy closes for followers (any user with active followers)
  async triggerCopyCloses(closeData) {
    try {
      console.log(`[DEBUG] triggerCopyCloses called with:`, closeData)
      // Trigger for any user who has active followers (not admin-only)
      const followerCheck = await prisma.$queryRaw`
        SELECT COUNT(*)::int AS cnt FROM copy_trading_followers
        WHERE leader_id = ${this.userId} AND status = 'ACTIVE'
      ...
      console.log(`[DEBUG] User ${this.userId} active followers: ${followerCheck?.[0]?.cnt}`)
      if (!hasFollowers) {
        console.log(`[DEBUG] No active followers, skipping copy closes`)
        return
      }
      ...
      console.log(`[DEBUG] Calling CopyTradingService.executeCopyCloses...`)
      const result = await CopyTradingService.executeCopyCloses(this.userId, {
        leaderPositionId,
        profitLossPercent,
        symbol,
        closeType: 'MARKET'
      })
      console.log(`[DEBUG] executeCopyCloses result:`, result)
    } catch (err) {
      // Fail silently - copy trading errors should not affect leader's trade
      console.error('[CopyTrading] Error triggering copy closes:', err?.message)
      console.error('[CopyTrading] Full error stack:', err.stack)
    }
  }
  ...
  async getTradeHistory(limit = 100) {
    ...
    return prisma.$queryRaw`
      SELECT t.*, a.symbol, a.name, a.type, t.created_at AS executed_at,
             p.realized_pnl, p.status AS position_status, p.exit_price
      FROM trades t
      JOIN assets a ON t.asset_id = a.id
      LEFT JOIN trading_positions p ON t.position_id = p.id
