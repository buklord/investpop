...
export class TradeService {
  ...
  async executeBuy(params) {
    ...
    if (existingPosition) {
      ...
      await prisma.$executeRaw`
        ...
        VALUES (
          ${positionId}::uuid, ${this.userId}::uuid, ${asset.id}::uuid, 'LONG',
    ...
    await prisma.$executeRaw`
      ...
      VALUES (
        ${tradeId}::uuid, ${this.userId}::uuid, ${asset.id}::uuid, ${positionId}::uuid,
  ...
  async executeSell(params) {
    ...
    await prisma.$executeRaw`
      ...
      VALUES (
        ${tradeId}::uuid, ${this.userId}::uuid, ${asset.id}::uuid, ${existingPosition.id}::uuid,
    ...
    if (remainingQuantity <= 0.0000001) { // Close position entirely (handle floating point)
      ...
      await prisma.$executeRaw`
        ...
        WHERE id = ${existingPosition.id}::uuid
      `
    } else {
  ...
  async getTradeHistory(limit = 100) {
    return prisma.$queryRaw`
      ...
      WHERE t.user_id = ${this.userId}::uuid
    ...
    if (status === 'open') {
      if (symbol) {
        return prisma.$queryRaw`
          ...
          WHERE p.user_id = ${this.userId}::uuid 
          AND p.status = 'OPEN' 
          AND a.symbol = ${symbol}
      ...
      return prisma.$queryRaw`
        ...
        FROM trading_positions p
        JOIN assets a ON p.asset_id = a.id
        WHERE p.user_id = ${this.userId}::uuid AND p.status = 'OPEN'
        ORDER BY p.opened_at DESC
      `
      ...
      return prisma.$queryRaw`
        ...
        FROM trading_positions p
        JOIN assets a ON p.asset_id = a.id
        WHERE p.user_id = ${this.userId}::uuid AND p.status = 'CLOSED'
  ...
  async createLimitOrder(params) {
    ...
    await prisma.$executeRaw`
      ...
      VALUES (
        ${id}::uuid, ${this.userId}::uuid, ${asset.id}::uuid, 
        'LIMIT', ${action}, ${quantity}, ${limitPrice},
  ...
  async getPendingOrders() {
    return prisma.$queryRaw`
      ...
      WHERE o.user_id = ${this.userId}::uuid AND o.status = 'PENDING'
  ...
  async cancelOrder(orderId) {
    await prisma.$executeRaw`
      ...
      WHERE id = ${orderId}::uuid AND user_id = ${this.userId}::uuid
    `
    return { success: true }
  }
}
