...
export async function signMessage(message, provider) {
  ...
  // For WalletConnect v2: use signClient directly to avoid EthereumProvider.request() chainId validation bug
  if (isWalletConnect && provider.signer && provider.session) {
    try {
      console.log('[walletConnect] using signClient directly for personal_sign...')
      const topic = provider.session.topic
      const chainId = provider.session.namespaces.eip155.chains?.[0] || 'eip155:1'
      console.log('[walletConnect] signClient topic:', topic, 'chainId:', chainId)

      signature = await provider.signer.request({
        topic,
        chainId,
        request: {
      ...
      console.log('[walletConnect] signClient personal_sign succeeded')
    } catch (err) {
      console.error('[walletConnect] signClient personal_sign failed:', err?.message || err)
      throw err
    }
  } else {
