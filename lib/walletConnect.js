...
export async function connectWallet() {
  ...
  try {
    ...
    // CRITICAL: Use a chain that the wallet actually approved in the session
    ...
    // Extract numeric chainId from session - use session chain NOT hardcoded 1
    let resolvedChainId = wc.chainId
    if (sessionChains.length > 0) {
      // Prefer mainnet (1) if available, otherwise use first approved chain
      ...
      console.log('[walletConnect] using chain from session:', preferredChain, '→', resolvedChainId)
    }
    if (!resolvedChainId) resolvedChainId = 56 // fallback to BSC which Trust Wallet always has
...
export async function signMessage(message, provider) {
  ...
  if (isWalletConnect) {
    ...
    console.log('[walletConnect] session chains:', sessionChains)
    if (sessionChains.length > 0 && provider.setDefaultChain) {
      const mainnetChain = sessionChains.find(c => c === 'eip155:1')
      const preferredChain = mainnetChain || sessionChains[0]
      const chainNum = parseInt(preferredChain.split(':')[1], 10)
      if (provider.chainId !== chainNum) {
        console.log('[walletConnect] fixing chainId to', chainNum, 'before signing')
        provider.setDefaultChain(chainNum)
      }
    }
