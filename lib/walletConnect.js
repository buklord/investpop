...
export async function signMessage(message, provider) {
  ...
  // Try personal_sign first (MetaMask standard)
  try {
    console.log('[walletConnect] trying personal_sign...')
    ...
    console.log('[walletConnect] personal_sign failed:', err1?.message || err1)

    // Fallback: try eth_sign (some wallets only support this)
    if (isWalletConnect) {
      try {
        console.log('[walletConnect] trying eth_sign fallback...')
        signature = await provider.request({
          method: 'eth_sign',
          params: [address, hexMessage],
        })
        console.log('[walletConnect] eth_sign succeeded')
      } catch (err2) {
        console.error('[walletConnect] eth_sign also failed:', err2?.message || err2)
