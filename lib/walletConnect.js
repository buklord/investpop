...
export async function signMessage(message, provider) {
  ...
  if (isWalletConnect) {
    console.log('[walletConnect] FULL session namespaces:', JSON.stringify(provider.session?.namespaces))
  }

  let signature

  // Try hex message first, fall back to plain string
  try {
    console.log('[walletConnect] trying personal_sign with hex message...')
    signature = await provider.request({
      method: 'personal_sign',
      params: [hexMessage, address],
    })
    console.log('[walletConnect] personal_sign (hex) succeeded')
  } catch (err1) {
    console.log('[walletConnect] personal_sign (hex) failed:', err1?.message || err1)
    try {
      console.log('[walletConnect] retrying personal_sign with plain string...')
      signature = await provider.request({
        method: 'personal_sign',
        params: [message, address],
      })
      console.log('[walletConnect] personal_sign (plain) succeeded')
    } catch (err2) {
      console.error('[walletConnect] personal_sign (plain) also failed:', err2?.message || err2)
      throw err1
    }
  }
